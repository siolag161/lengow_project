...
@django_rq.job
def parse_orders_from_xml(url):
    res = requests.get(url)
    res.encoding = 'utf-8'
    if res.status_code == 200:
        orders = LengowOrderXMLParser.parse_and_save(res.text)
        return "Retrieved and saved with success : %s orders" % (len(orders))
    else:
        return "Task aborted"
...
class LengowOrderXMLParser(object):
    ...
    def manual_update_orders(cls, orders=None):
        orders = cls.to_list(orders) or Order.objects.all()
